...
export default function DistrictPage() {
  ...
  const seoData = useMemo(() => {
    ...
    const seoDescription = `★4.98 из 5 (1240 отзывов) Химчистка мебели ${d.nameGen} Краснодара с выездом на дом. Диваны, кресла, матрасы, ковры. Сушка 2–4 часа. Недорого. ☎ ${PHONE_DISPLAY}`;
    const n = d.name;
    const keywords = [
      `химчистка мебели ${n}`,
    ...
    const jsonLd = {
      ...
      telephone: PHONE,
      url: `${SITE_URL}/himchistka-${d.slug}`,
  ...
  const otherDistricts = useMemo(
    () => DISTRICTS.filter((x) => x.slug !== d.slug).slice(0, 6),
    [d.slug],
  );
  ...
  return (
    <div className="min-h-screen" style={{ background: "var(--light-bg)" }}>
      ...
      <Seo
        ...
        canonical={`${SITE_URL}/himchistka-${d.slug}`}
      />
