...
export default function ServiceDistrictPage() {
  ...
  // 🚫 404 если нет услуги или района
  if (!service || !d) {
  ...
  const seoData = useMemo(() => {
    ...
    const seoDescription = `★4.98 из 5 (1240 отзывов) ${service.title} ${d.nameGen} Краснодара с выездом на дом. ${service.seoDescription?.split(".")[1] ?? "Быстро, качественно, без предоплаты."} ☎ ${PHONE_DISPLAY}`;
    const keywords = [
      `${service.title.toLowerCase()} ${d.name}`,
      `${service.title.toLowerCase()} ${d.name} краснодар`,
      `${service.title.toLowerCase()} ${d.name} на дому`,
      `${service.title.toLowerCase()} ${d.name} цена`,
      `${service.title.toLowerCase()} ${d.name} недорого`,
      `${service.title.toLowerCase()} ${d.name} выезд`,
      `химчистка мебели ${d.name}`,
      `химчистка мебели ${d.name} краснодар`,
    ].join(", ");

    const jsonLd = {
      "@context": "https://schema.org",
      "@type": "Service",
      name: `${service.title} ${d.nameGen}`,
    ...
    return { seoTitle, seoDescription, keywords, jsonLd };
  }, [service, d]);
  ...
  const otherDistricts = useMemo(
    () => DISTRICTS.filter((x) => x.slug !== d.slug).slice(0, 4),
    [d.slug],
  );

  const { seoTitle, seoDescription, keywords, jsonLd } = seoData;
  ...
  return (
    <div className="min-h-screen" style={{ background: "var(--light-bg)" }}>
      ...
      <Seo
        ...
        description={seoDescription}
        keywords={keywords}
        path={`/uslugi/${service.slug}/${d.slug}`}
        image={service.img}
        jsonLd={jsonLd}
        breadcrumbs={[
        ...
        canonical={`${SITE_URL}/uslugi/${service.slug}/${d.slug}`}
      />
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 py-6 md:py-12">
        ...
        <div className="grid md:grid-cols-2 gap-4 md:gap-8 mb-8 md:mb-12 items-center">
          <div>
            ...
            >
              {service.lead} Работаем {d.nameGen} — выезжаем на дом без доплат.
            </p>
        ...
        {/* 📍 О районе */}
        <div
          className="bg-white rounded-2xl p-5 sm:p-6 shadow-sm mb-8 md:mb-12 flex gap-4 items-start"
        ...
        >
          ...
          >
            <Suspense fallback={<span>📍</span>}>
              <Icon name="MapPin" size={20} style={{ color: "var(--teal)" }} />
            </Suspense>
          </div>
          <div>
        ...
        <div className="mb-12">
          <h2
            ...
            style={{ color: "var(--dark)" }}
          >
            Стоимость {service.title.toLowerCase()} {d.nameGen}
          </h2>
          <div
        ...
        >
          <h2 className="font-oswald font-bold text-white text-2xl md:text-3xl mb-3">
            Заказать {service.title.toLowerCase()} {d.nameGen}
          </h2>
        ...
        {/* 🗺️ Эта услуга в других районах */}
        <div className="mb-12">
          <h2
            className="font-oswald font-bold text-2xl mb-6"
            style={{ color: "var(--dark)" }}
          >
            {service.title} в других районах
          </h2>
          <div className="grid sm:grid-cols-2 md:grid-cols-4 gap-3">
            {otherDistricts.map((od) => (
              <Link
                ...
                className="bg-white rounded-2xl p-4 shadow-sm flex items-center gap-3 transition-all hover:shadow-md hover:-translate-y-0.5"
                style={{ border: "1px solid var(--border)" }}
              >
                <Suspense fallback={<span>📍</span>}>
                  <Icon
            ...
            {otherServices.map((s) => (
              ...
              >
                <div className="aspect-[4/3] overflow-hidden relative">
                  ...
                  />
                  {s.badge && (
                    <span className="absolute top-3 left-3 section-tag text-xs">
                      {s.badge}
                    </span>
                  )}
                </div>
